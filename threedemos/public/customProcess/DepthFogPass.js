        ...
        return new THREE.ShaderMaterial( {
            ...
            fragmentShader:
                ...
                void main() {
                    ...
                    vec3 wP = cameraPosition + (cameraVec)*abs(clipZ);

                    float dis = distance(cameraPosition, wP);
                    vec4 linearFogColor = mix(vec4(fogColor, 0.0), vec4(fogColor, 0.0), dis/fogLinearDis);
                 
                    gl_FragColor = mix(texture2D( colorTexture, vUv), vec4(fogColor, 1.0), dis/fogLinearDis);
