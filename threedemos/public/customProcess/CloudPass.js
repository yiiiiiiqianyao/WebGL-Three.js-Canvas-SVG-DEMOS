THREE.CloudPass = function(scene, camera) {
    THREE.Pass.call( this );

    this.scene = scene;
	this.camera = camera;

    this.cloudMaterial = this.getGrayMaterial( );
    this.fsQuad = new THREE.Pass.FullScreenQuad( this.cloudMaterial );
}

THREE.CloudPass.prototype = Object.assign( Object.create( THREE.Pass.prototype ), {
    constructor: THREE.CloudPass,
    render: function(renderer, writeBuffer, readBuffer, deltaTime, maskActive) {
        
        this.cloudMaterial.uniforms[ "colorTexture" ].value = readBuffer.texture;
    ...
    getGrayMaterial: function() {
        return new THREE.ShaderMaterial( {
            ...
            fragmentShader:
                ...
                uniform float time;
                void main() {
                    vec3 p = vec3(vUv, time*speed/2.0);
                    ...
                    // 卢马换算公式光度; 
					float a = 0.2126*q + 0.7152*q + 0.0722*q;
                    // diff += vec4(q, q, q, a);
                    // gl_FragColor = diff;
                    gl_FragColor = mix(diff, vec4(q, q, q, 1.0), 0.5);
                }
                `
        })
    }
