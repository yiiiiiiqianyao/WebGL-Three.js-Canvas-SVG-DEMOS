...
THREE.DepthFogPass = function(scene, camera, options) {
    ...
    // this.depthTarget.depthTexture.anisotropy = 64
        ...
        return new THREE.ShaderMaterial( {
            ...
            fragmentShader:
                ...
                void main() {
                    ...
                    vec3 worldPosition = cameraPos + (cameraVec)*abs(clipZ);

                    float dis = distance(cameraPos, worldPosition);
                    vec4 linearFogColor = mix(vec4(fogColor, 0.0), vec4(fogColor, 0.0), dis/fogLinearDis);
                 
                
                    float skyHeight = 50.0; // 天空距离地面的高度
                    vec4 skyColor = vec4(0.8, 1.0, 1.0, 1.0);
                    vec4 groundColor = vec4(0.2, 0.2, 0.2, 1.0);

                    if(dis/cameraFar > 1.0) { // 绘制天空盒子的部分

                        if(worldPosition.y > skyHeight) { // 天空部分

                            gl_FragColor = skyColor;

                        } else if(worldPosition.y > 0.0) { // 地平线和天空的过度部分 - 默认的地平线高度

                            // gl_FragColor = vec4(vec3(worldPosition.y/skyHeight), 1.0);
                            gl_FragColor = mix(skyColor, groundColor, 1.0 - worldPosition.y/skyHeight);
                        ...
                        //gl_FragColor = vec4(vec3(dis/cameraFar), 0.0);
                        discard;
                    }
